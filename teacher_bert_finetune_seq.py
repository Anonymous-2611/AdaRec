...
def start(start_metric=(0.0, 0.0)):
    best_mrr5, best_hit5 = start_metric
...
if __name__ == "__main__":
    ...
    # transfer task into seq, pretrained task is MLM
    data_provider = SequenceLoaderProvider(args)
    args.loader_num_aux_vocabs = 2  # IMPORTANT
    ...
    if args.resume is not None:
        resume_model_path = Path(args.resume).joinpath("best-model.pth")
        state_dict = load_state_dict(resume_model_path)

        model_sd = state_dict["state_dict"]["model"]

        model.load_state_dict(model_sd)
        logging.info("Checkpoint loaded, testing...")
        start_metric = do_validate()
    else:
